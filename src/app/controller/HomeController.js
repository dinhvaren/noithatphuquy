const { User, Product, Category, CategoryParent } = require("../models/index");
...
class HomeController {
  ...
  async productDetails(req, res, next) {
    try {
      const { slug  } = req.params; // id truyền qua URL /products/:id
      ...
      res.render("pages/ProductDetails", {
        page: { title: `Chi tiết sản phẩm - ${product.name}` },
        product,
        user: req.user,
      });
    } catch (err) {
        ...
        if (Array.isArray(category)) {
          ...
          for (let catId of category) {
            ...
            if (childCategories.length > 0) {
              categoriesToFind.push(
                ...childCategories.map((c) => c._id.toString())
              );
            } else {
              categoriesToFind.push(catId);
            }
          }
          filter.categoryId = { $in: categoriesToFind };
        } else {
          // chỉ chọn 1 ID
          const childCategories = await Category.find({
            parent_id: category,
          ...
          if (childCategories.length > 0) {
            filter.categoryId = {
              $in: childCategories.map((c) => c._id.toString()),
            };
          } else {
